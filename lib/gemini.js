    ...
    for (const model of GEMINI_MODELS) {
        try {
            ...
            if (!response.ok) {
                ...
                console.warn(`[AI] Model ${model} failed with status ${response.status}: ${compactErrorText.slice(0, 200)}...`);
                
                // Preserve the provider message so the UI can tell the user whether
                // this is a bad key, disabled API, quota issue, or model access issue.
                lastError = new Error(
                    compactErrorText
                        ? `Model ${model} failed: ${response.status} ${response.statusText} - ${compactErrorText.slice(0, 300)}`
                        : `Model ${model} failed: ${response.status} ${response.statusText}`
                );
                continue;
            }
