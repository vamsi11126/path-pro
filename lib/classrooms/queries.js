...
import { getClassroomRewardSnapshot } from '@/lib/classrooms/rewards'
...
export async function getTeacherClassroomDetail(supabase, classroomId, teacherUserId) {
  ...
  const rewards = await getClassroomRewardSnapshot(supabase, {
    classroomId,
    classroomTimeZone: classroom.timezone,
    members: mappedMembers
  })
  const activeBadge = rewards.activeBadge
  ...
  return {
    ...
    members: mappedMembers.map((member) => ({
      ...member,
      isActiveBadgeHolder: activeBadge?.winnerStudentUserId === member.student_user_id,
      activeBadgeMeta: activeBadge?.winnerStudentUserId === member.student_user_id ? activeBadge : null
    })),
  ...
  const students = activeMembers
    .map((member) => {
      ...
      return {
        ...
        isActiveBadgeHolder: activeBadge?.winnerStudentUserId === studentUserId,
        activeBadgeMeta: activeBadge?.winnerStudentUserId === studentUserId ? activeBadge : null,
        totalMinutes,
