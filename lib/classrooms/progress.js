...
import { isSameDateInTimeZone, normalizeTimeZone } from '@/lib/classrooms/time'
...
export async function submitClassroomReview(supabase, { classroomId, classroomCourseId, studentUserId, topicId, quality, durationMinutes = 0 }) {
  ...
  const scheduledReviewAt = topic.progress.next_review_at || null
  const classroomTimeZone = normalizeTimeZone(classroomCourse.classrooms?.timezone)
  const reviewCompletedOnTime = scheduledReviewAt
    ? isSameDateInTimeZone(new Date(), scheduledReviewAt, classroomTimeZone)
    : null
  ...
  const { error: logError } = await supabase
    ...
    .insert({
      ...
      source_type: 'classroom',
      scheduled_review_at: scheduledReviewAt,
      review_completed_on_time: reviewCompletedOnTime
    })
