...
function buildAssessmentPayload(classroomId, teacherUserId, payload) {
  ...
  return {
    ...
    created_via: normalizeText(payload.createdVia || payload.created_via || payload.creationMethod) || 'manual',
...
export async function getTeacherAssessmentDetail(supabase, classroomId, assessmentId, teacherUserId) {
  ...
  const { data: members, error: membersError } = studentIds.length > 0
    ? await reader
        .from('classroom_members')
        .select(`
          id,
          student_user_id,
          profile:profiles!classroom_members_student_user_id_fkey (
            id,
            full_name,
            username,
            education_level
  ...
  const memberByUserId = new Map((members || []).map((member) => [member.student_user_id, member]))
